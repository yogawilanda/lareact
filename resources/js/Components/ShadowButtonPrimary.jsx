...
function ShadowButtonPrimary({ onPress, href, children }) {
    return (
        <Button variant='shadow' color='primary' onPress={onPress ? onPress : () => console.log("Unidentified Button Action")}>
            <Link href={href ? href : "/"} className='text-white dark:text-white'>
                {children}
            </Link>
        </Button>
    );
